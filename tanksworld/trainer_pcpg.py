...
from algos.torch_ppo.pcpg import PCPGPolicy
...
from algos.torch_ppo.utils.normalizer import *
...
class Trainer:
        ...
        if config['num_rollout_threads'] == 1:
                    ...
                    if seed_idx == config['seed_idx']:
                        ...
                        if config['eval_mode']:
                            policy_record = PolicyRecord(local_folder_name, './logs/' + config['eval_logdir'] + '/',
                                                         std=False, intrinsic_reward=True)
                        else:
            ...
            for p_seed_idx, p_seed in enumerate(self.policy_seeds):
                ...
                if seed_idx == config['seed_idx']:
                    # Set folder name and policy record to plot the rewards
                    local_folder_name = folder_name + '/seed{}'.format(seed_idx)
                    policy_record = PolicyRecord(local_folder_name, './logs/' + config['logdir'] + '/', std=False,
    ...
    def get_policy_params(self):
        ...
        policy_kwargs = {
            ...
            'start_exploit': 10,
            #'start_exploit': 0,
            'n_rollouts_for_density_est': 1,
            #'n_rollouts_for_density_est': 1,
            'reward_normalizer': RescaleNormalizer(),
            'reward_bonus_normalizer': RescaleNormalizer(),
            'ridge': 0.01,
            'n_policy_loops': 1,
            #'n_policy_loops': 100,
            #'n_traj_per_loop': 1,
            ...
            'bonus_coeff': 0.001,
            'entropy_weight': config['entropy_coef'],
            'ppo_ratio_clip': config['clip_ratio'],
            'cnn_model_path': config['cnn_path'] if config['cnn_path'] != 'None' else None,
            'num_envs': config['n_eval_seeds'] if config['eval_mode'] else config['num_rollout_threads'],
...
if __name__ == '__main__':
    ...
    if args['eval_mode'] or args['visual_mode'] or args['data_mode']:
        ...
        for seed_idx, policy_record in enumerate(policy_records):
            ...
            policy_params = trainer.get_policy_params()
            #model_path = './logs/selfplay-ckpt/499999.pth'
            policy_params['model_path'] = model_path
            ...
            callback = EvalCallback(envs[seed_idx], policy_record, val_env=val_env, eval_env=eval_envs[seed_idx],
                                    eval_steps=100)
            policy = PCPGPolicy(envs[seed_idx], callback, False, **policy_params)
