...
from tanksworld.env_centralized.minimap_util import (
    minimap_for_player,
    displayable_rgb_map,
    display_cvimage,
)
...
class TanksWorldEnv(gym.Env):
    ...
    def __init__(
        ...
        if input_type == "stacked":
            self.observation_space = gym.spaces.Box(
                0, 255, (self.n_enable_inputs * 4, 128, 128)
            )
        elif input_type == "dict":
            self.observation_space = gym.spaces.Dict(
                {
                    str(tank_idx): gym.spaces.Box(0, 255, (4, 128, 128))
                    for tank_idx in self.enable_input_tanks
                }
            )
        else:
        ...
        self.image_scale = (
            image_scale  # scale the images going to the tanks (will be scaled from 128)
        )
        self.penalties = friendly_fire  # include negative rewards for friendly fire, neutral tanks, etc
        self.take_damage_penalty = (
            take_damage_penalty  # penalize getting hit by someone else
        )
        ...
        self.barrier_img = cv2.imread(
            os.path.join(path_name, "obstaclemap_fixed.png"), 1
        )
    ...
    def process_state(self, ret_states):
        ...
        if self.input_type == "stacked":
            ...
            states = {
                str(tank_idx): states[i].squeeze()
                for i, tank_idx in enumerate(self.enable_input_tanks)
            }
        return states
    ...
    def get_state(self):
        ...
        ret_states = [
            minimap_for_player(state_reformat, i, barriers)
            for i in self.enable_input_tanks
        ]
        ...
        if self.image_scale != 128:
            ret_states = [
                cv2.resize(s, (self.image_scale, self.image_scale)) for s in ret_states
            ]
    ...
    def reset(self, **kwargs):
        ...
        if not TanksWorldEnv._env:
            try:
                ...
                TanksWorldEnv._env = UnityEnvironment(
                    file_name=self._filename,
                    worker_id=self._workerid,
                    seed=np.random.randint(6500),
                    timeout_wait=500,
                )
        ...
        self._env_info = self._env.reset(train_mode=0, config=params)[
            self._default_brain
        ]
    ...
    def is_done(self, state):
        ...
        training_health = [
            i * TanksWorldEnv._tank_data_len + 3 for i in self.training_tanks
        ]
                ...
                if isinstance(obj, dict):
                    for kk in obj:
                        ...
                        self.tb_writer.add_scalar(
                            prefix + k + "_" + kk, sub_obj, self.log_iter
                        )
                else:
    ...
    def update_tank_stats(self, i, state, dhealth, ally_stats, enemy_stats, new_shot):
        ...
        if damage_dealt > 0.0:
            ...
            if team_hit_type == "ally":
                ...
                ally_stats["reward_components_cumulative"][
                    "take_damage_penalty"
                ] += delta_rew
                ally_stats["reward_components_cumulative"][
                    "penalties_only"
                ...
                delta_rew = -(damage_dealt / 100.0)
                enemy_stats["reward_components_cumulative"][
                    "take_damage_penalty"
                ] += delta_rew
                enemy_stats["reward_components_cumulative"][
                    "penalties_only"
    ...
    def objectives(self):
        ...
        for i in range(10):
            state = self._env_info.vector_observations[0][
                i
                * TanksWorldEnv._tank_data_len : (i + 1)
                * TanksWorldEnv._tank_data_len
            ]
...
if __name__ == "__main__":
    ...
    env = TanksWorldEnv(
        exe, timeout=500, will_render=True, speed_red=1.0, speed_blue=0.1
    )
