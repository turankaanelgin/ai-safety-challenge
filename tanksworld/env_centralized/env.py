...
class TanksWorldEnv(gym.Env):
    ...
    def process_state(self, ret_states):
        ...
        if self.input_type == 'stacked': 
            ...
            states = {str(tank_idx): states[i].squeeze() for i, tank_idx in enumerate(self.enable_input_tanks)}
        return states
    ...
    def reset(self,**kwargs):
        ...
        if not TanksWorldEnv._env:
            try:
                ...
                TanksWorldEnv._env = UnityEnvironment(file_name=self._filename, worker_id=self._workerid, seed=np.random.randint(6500),timeout_wait=500)
