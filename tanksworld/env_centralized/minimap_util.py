...
def draw_tanks_in_channel(tank_data, reference_tank):
    ...
    for td in tank_data:
        ...
        rel_x, rel_y = point_relative_point_heading(
            [td[0], td[1]], reference_tank[0:2], reference_tank[2]
        )
        ...
        if len(td) > 4:
            ...
            if bx < 900:
                rel_x, rel_y = point_relative_point_heading(
                    [bx, by], reference_tank[0:2], reference_tank[2]
                )
                x = (rel_x / UNITY_SZ) * SCALE + float(IMG_SZ) * 0.5
                y = (rel_y / UNITY_SZ) * SCALE + float(IMG_SZ) * 0.5
...
def barriers_for_player(barriers, reference_tank):
    ...
    wleft_rel = np.asarray(
        [points_relative_point_heading(wleft, reference_tank[0:2], reference_tank[2])]
    )
    ...
    wright_rel = np.asarray(
        [points_relative_point_heading(wright, reference_tank[0:2], reference_tank[2])]
    )
    ...
    wtop_rel = np.asarray(
        [points_relative_point_heading(wtop, reference_tank[0:2], reference_tank[2])]
    )
    ...
    wbot_rel = np.asarray(
        [points_relative_point_heading(wbot, reference_tank[0:2], reference_tank[2])]
    )
...
def minimap_for_player(tank_data_original, tank_idx, barriers):
    ...
    ret = (
        np.asarray(
            [ally_channel, neutral_channel, enemy_channel, barriers_channel]
        ).astype(np.float32)
        / 255.0
    )
