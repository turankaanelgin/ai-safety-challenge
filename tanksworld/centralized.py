...
if args.eval_mode:
    ...
    kwargs_1 = {'static_tanks': [], 'random_tanks': [5, 6, 7, 8, 9], 'disable_shooting': [],
                ...
                'log_statistics': False, 'no_timeout': False, 'barrier_heuristic': False}


if args.eval_mode:
    kwargs_2 = {1: {'eval_mode': True, 'model_path': './models/friendly-fire-0.8-replicated/4000000.pth'},
                2: {'eval_mode': True, 'model_path': './models/friendly-fire-0.8-replicated-v2/4000000.pth'},
    ...
    kwargs_2 = {
                1: {'steps_per_epoch': 64, 'train_pi_iters': 4, 'train_v_iters': 4, 'actor_critic': MLPActorCritic,
                    'ac_kwargs': {'hidden_sizes': (64, 64)}, 'neg_weight_constant': 0.8, 'model_id': 'friendly-fire-0.8-replicated',
                ...
                2: {'steps_per_epoch': 64, 'train_pi_iters': 4, 'train_v_iters': 4, 'actor_critic': MLPActorCritic,
                    'ac_kwargs': {'hidden_sizes': (64, 64)}, 'neg_weight_constant': 0.8, 'model_id': 'friendly-fire-0.8-replicated-v2',
                    'model_path': './models/frozen-cnn-0.8/4000000.pth'},
                3: {'steps_per_epoch': 64, 'train_pi_iters': 4, 'train_v_iters': 4, 'actor_critic': MLPActorCritic,
                    'ac_kwargs': {'hidden_sizes': (64, 64)}, 'neg_weight_constant': 0.8, 'model_id': 'friendly-fire-0.8-replicated-v3',
                    'model_path': './models/frozen-cnn-0.8/4000000.pth'},
...
video = cv2.VideoWriter("output.avi", cv2.VideoWriter_fourcc(*"MJPG"), 30, (128, 128))

for i in range(100):
    env.step(list(np.random.rand(5,3)))
    state = (env.get_state())
    #cv2.imwrite(str(i)+'.png', state)
    video.write(state)
video.release()
