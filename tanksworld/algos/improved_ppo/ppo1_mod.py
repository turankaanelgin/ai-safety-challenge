...
from algos.improved_ppo.seg_gen_limited import traj_segment_generator
...
class PPO1(ActorCriticRLModel):
    ...
    def __init__(self, policy, env, comm, gamma=0.99, timesteps_per_actorbatch=256, clip_param=0.2, entcoeff=0.01,
                 ...
                 curriculum_penalty_weight=1.0, anneal_penalty_weight=0.0):
    ...
    def evaluate(self, total_timesteps, policy_record=None):
    ...
    def learn(self, total_timesteps, policy_record=None, callback=None, seed=None, log_interval=100, tb_log_name="PPO1",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            with self.sess.as_default():
                ...
                # First we generate the trajectory
                ...
                while True:
                    ...
                    if timesteps_so_far % 10000 == 0:
                        if policy_record is not None:
                            ...
                            self.save(policy_record.data_dir + "ppo_save_{}".format(timesteps_so_far))

                    if self.anneal_penalty_weight is not None:
                        if (timesteps_so_far+1) % int(4e6 / self.anneal_penalty_weight) == 0:
                            if self.env.penalty_weight < self.curriculum_penalty_weight:
                                self.env.penalty_weight = self.env.penalty_weight + self.anneal_penalty_weight
                    ...
                    if local_steps_remaining > self.timesteps_per_actorbatch:
                        seg = seg_gen.__next__()  # Get the next segment
                    ...
                    # The segment consists of observations, actions, advantage function
                    observations, actions = seg["observations"], seg["actions"]
                    atarg, tdlamret = seg["adv"], seg["tdlamret"]

                    # true_rew is the reward without discount
                    # Save the reward of the segment
    ...
    def save(self, save_path, cloudpickle=False):
        data = {
            ...
            "anneal_penalty_weight": self.anneal_penalty_weight,
            "curriculum_penalty_weight": self.curriculum_penalty_weight,
    ...
    # overwriting load method to require comms
    @classmethod
