...
class RolloutBuffer:
    ...
    def __init__(self, obs_dim, act_dim, size, gamma=0.99, lam=0.95, n_envs=1, use_sde=False,
        ...
        if use_rnn:
            ...
            self.next_obs_buf = torch.zeros(core.combined_shape_v3(size, n_envs, 5, obs_dim)).to(device)
    ...
    def store(self, obs, act, rew, val, logp, entropy, next_obs):
        """
...
class PPOPolicy():
    ...
    def setup_model(self, actor_critic, pi_lr, vf_lr, pi_scheduler, vf_scheduler, ac_kwargs):
        ...
        self.ac_model = actor_critic(self.env.observation_space, self.env.action_space, **ac_kwargs).to(device)
        self.icm = core.ICM(self.env.observation_space, self.env.action_space).to(device)
        self.pi_optimizer = Adam(list(self.ac_model.pi.parameters()) + list(self.icm.parameters()), lr=pi_lr)
    ...
    def compute_loss_icm(self, data):
        obs, act, next_obs = data['obs'], data['act'], data['next_obs']
        obs, act, next_obs = obs.to(device), act.to(device), next_obs.to(device)
        obs = torch.flatten(obs, end_dim=2)
        act = torch.flatten(act, end_dim=2)
        next_obs = torch.flatten(next_obs, end_dim=2)
        next_obs_real, next_obs_pred, action_pred = self.icm(obs, act, next_obs)
        forward_pred_error = ((next_obs_real - next_obs_pred) ** 2).mean()
        inverse_pred_error = ((act - action_pred) ** 2).mean()
        return 0.5 * forward_pred_error + 0.5 * inverse_pred_error


    def update(self, buf, train_pi_iters, train_v_iters, target_kl, clip_ratio, step):
    ...
    def learn(self, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=-1,
        ...
        while step < steps_to_run:
            ...
            if use_rnn:
                ...
                buf.store(state_history, a, r, v, logp, entropy, None)
            else:
                buf.store(obs, a, r, v, logp, entropy, self.obs)
