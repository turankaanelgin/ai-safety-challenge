...
class EvalCallback:
    ...
    def __init__(self, env, policy_record, eval_steps=10, val_env=None, eval_env=None, eval_env_with_enemy=None):
        self.env = env
        self.model = None
        self.enemy_model = None
        self.aggressive_enemy_model = None
        self.policy_record = policy_record
        self.val_env = val_env
        self.eval_env = eval_env
        self.eval_env_with_enemy = eval_env_with_enemy
        self.eval_steps = eval_steps

    def init_model(self, model):
        self.model = model

    def init_enemy_model(self, enemy_model):
        self.enemy_model = enemy_model

    def init_aggressive_enemy_model(self, aggressive_enemy_model):
        self.aggressive_enemy_model = aggressive_enemy_model
    ...
    def evaluate_policy_against_enemy(self, model_state_dict, device, step=0, aggressive=False, save_files=True):

        episodes = 0
        observation = self.eval_env_with_enemy.reset()
        ...
        while episodes < self.eval_steps:
            ally_obs = observation[:, :5, :, :, :]
            with torch.no_grad():
                ally_a, v, logp, entropy = self.model.step(torch.as_tensor(ally_obs, device=device))
            enemy_obs = observation[:, 5:, :, :, :]
            if aggressive:
                with torch.no_grad():
                    enemy_a, _, _, _ = self.aggressive_enemy_model.step(torch.as_tensor(enemy_obs, device=device))
            else:
                with torch.no_grad():
                    enemy_a, _, _, _ = self.enemy_model.step(torch.as_tensor(enemy_obs, device=device))
            action = torch.cat((ally_a, enemy_a), dim=1)
            observation, reward, done, info = self.eval_env_with_enemy.step(action.cpu().numpy())
            curr_done = [done[idx] or curr_done[idx] for idx in range(num_envs)]
            ...
            if np.all(curr_done):
                ...
                if episodes % 2 == 0 and episodes > 0 and save_files:

                    if aggressive:
                        file_name = os.path.join(self.policy_record.data_dir,
                                                 'mean_statistics_eval_enemy_aggressive_{}.json'.format(step))
                    else:
                        file_name = os.path.join(self.policy_record.data_dir,
                                                 'mean_statistics_eval_enemy_{}.json'.format(step))
    ...
    def evaluate_policy(self, model_state_dict, device, step=0, save_files=True):
        ...
        while episodes < self.eval_steps:
            ...
            if np.all(curr_done):
                ...
                if episodes % 2 == 0 and episodes > 0 and save_files:
                    ...
                    with open(os.path.join(self.policy_record.data_dir, 'all_statistics_eval.json'), 'w+') as f:
                        json.dump({'Number of games': episodes,
                    ...
                    with open(os.path.join(self.policy_record.data_dir, 'mean_statistics_per_env_eval.json'),
                              'w+') as f:
                        json.dump({'Number of games': episodes,
    ...
    def save_metrics_multienv(self, episode_returns, episode_lengths, episode_red_blue_damages, episode_red_red_damages,
                              ...
                              episode_intrinsic_rewards=None):
            ...
            if not eval_mode:
                for idx in range(len(episode_lengths)):
                    if episode_intrinsic_rewards:
                        ...
                        self.policy_record.add_result(np.average(episode_returns[idx]),
                                                      episode_lengths[idx])
                for idx in range(len(episode_red_red_damages)):
                    self.policy_record.add_damage_result(episode_red_blue_damages[idx],
