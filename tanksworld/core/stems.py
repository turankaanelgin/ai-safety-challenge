...
class UserStem(object):
	...
	def kickoff(self, match_list, policy_types, steps_per_match, entity_remaps=[], render=False, scale=False, 
		policy_kwargs={}, env_kwargs={}, plot_colors={}, policy_folder_names={}, env_seeds=[]):
		...
		#broadcast policy folder names
		self.global_comm.bcast(policy_folder_names, root=0)

		self.global_comm.bcast(env_seeds, root=0)
...
class WorkerStem(object):
	...
	def loop(self):
		...
		# receive the policy folder names
		policy_folder_names = self.global_comm.bcast(None, root=0)

		env_seeds = self.global_comm.bcast(None, root=0)
		...
		for midx, m in enumerate(match_list):
			...
			# Create the match idx --> environment seed mapping =======================
			next_seed = -1
			if env_seeds:
				next_seed = env_seeds[idx]
				idx += 1
				idx %= len(env_seeds)
				env_seed_map.append(next_seed)

			# Create the rank --> policy mapping =======================================
			policies_flat.append(-1) #one environment for each match
			policy_folder_map.append(None)
		...
		if self.global_rank not in unused_ranks:
			...
			my_pol_folder_name = policy_folder_map[self.global_rank]
			my_env_seed = env_seed_map[self.global_rank]
		else:
			#unused rank
			my_pol = -2
			my_match = -2
		...
		if my_pol == -1:
			...
			if env_seeds:
				my_env_kwargs['seed'] = my_env_seed
			...
			if policy_group_comm.Get_rank() == 0:
				if plot_colors:
					pr = PolicyRecord(my_pol, my_pol_folder_name, self.log_comms_dir, plot_colors[my_pol])
				else:
					pr = PolicyRecord(my_pol, my_pol_folder_name, self.log_comms_dir)
				pr.load()
			...
			#this is an unused p(roc(ess :(+1)+1)
