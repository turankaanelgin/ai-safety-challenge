...
def draw_bullet(image, x, y):
    cv2.circle(image, (int(x),int(y)), 1, 255.0, thickness=-1)
    return image

def draw_tanks_in_channel2(tank_data, barriers):
...
def minimap_for_player(tank_data_original, barriers):
    ...
    ally_channel = draw_tanks_in_channel2(ally, barriers)
    enemy_channel = draw_tanks_in_channel2(enemy, barriers)
    neutral_channel = draw_tanks_in_channel2(neutral, barriers)
