...
from tanksworld.env_rgb.minimap_util import minimap_for_player, displayable_rgb_map, display_cvimage
import cv2
import pathlib
...
class TanksWorldEnv(gym.Env):
    ...
    def __init__(self, exe, action_repeat=6, image_scale=128, timeout=500, friendly_fire=True, take_damage_penalty=True, kill_bonus=True, death_penalty=True,
        ...
        self.reward_stats = np.zeros((1000000, 5))
        #self.reward_stats[self.reward_stats_count] = [1,2,3,4,5]
    ...
    def get_state(self):
        ...
        #np.save('tmp/tank_info', state_reformat)
        #for i in range(2):
        #    state_reformat[i][3]=-1
        img = minimap_for_player(state_reformat, self.barrier_img)
        return img
        ...
        #ret_states = np.concatenate(ret_states, axis=1)
        #return ret_states
    ...
    def reset(self,**kwargs):
        ...
        if not TanksWorldEnv._env:
            try:
                ...
                TanksWorldEnv._env = UnityEnvironment(file_name=self._filename, worker_id=np.random.randint(10000),
...
if __name__ == "__main__":
    ...
    4: health
    '''
    tank_info = np.load('tmp/tank_info.npy')
    print(tank_info)
    barrier = cv2.imread('obstaclemap_fixed.png',1)
    #img = minimap_for_player(tank_info, barrier)
    #import pdb; pdb.set_trace();
    #Image.fromarray(img).save('tmp/tanks.png')
    sys.exit()
